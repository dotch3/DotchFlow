...
router.get('/', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const categories = await queryAll(db, 'SELECT * FROM categories WHERE user_id = ?', [req.userId]);
...
router.post('/', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const cat = await queryOne(db, 'SELECT * FROM categories WHERE id = ?', [lastInsertRowId]);
...
router.put('/:id', authMiddleware, async (req, res) => {
  ...
  try {
    const db = await getDatabase();
    const cat = await queryOne(db, 'SELECT * FROM categories WHERE id=? AND user_id=?', [req.params.id, req.userId]);
    ...
    const updated = await queryOne(db, 'SELECT * FROM categories WHERE id = ?', [req.params.id]);
...
router.delete('/:id', authMiddleware, async (req, res) => {
  ...
  try {
    const db = await getDatabase();
    const cat = await queryOne(db, 'SELECT id FROM categories WHERE id=? AND user_id=?', [req.params.id, req.userId]);
    if (!cat) return res.status(404).json({ error: await translate('categories.notFound') });
    await execute(db, 'DELETE FROM categories WHERE id = ?', [req.params.id]);
