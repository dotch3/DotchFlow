...
const { t, createTranslator, translateErrors } = require('../../i18n');
...
router.post(
  '/register',
  [
    body('email').isEmail().withMessage('errors.invalidEmail'),
    body('password').isLength({ min: 6 }).withMessage('errors.passwordMinLength'),
  ],
  async (req, res) => {
    ...
    try {
      ...
      for (const cat of defaultCategories) {
        await execute(db, 'INSERT INTO categories (user_id, name, icon, monthly_limit) VALUES (?, ?, ?, ?)',
          [lastInsertRowId, cat.name, cat.icon, cat.limit]);
      }

      const token = jwt.sign({ userId: lastInsertRowId }, JWT_SECRET, { expiresIn: '7d' });
      const user = await queryOne(db, 'SELECT id, email, language, xp_points, dotch_coins, level, streak_count FROM users WHERE id = ?', [lastInsertRowId]);
...
router.put('/language', require('../middleware/auth').authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const db = await getDatabase();
    await execute(db, 'UPDATE users SET language = ? WHERE id = ?', [language, req.userId]);
